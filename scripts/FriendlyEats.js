...
function FriendlyEats() {
  ...
  this.myButton = document.getElementById('signout');
  this.myButton.addEventListener('click', this.signOut.bind(this));
  ...
  messaging.onMessage(function(payload) {
    ...
    document.getElementById("collection").innerHTML = payload.notification.body;
  });
...
FriendlyEats.prototype.getFSData = function() {
  var db = firebase.firestore();
  db.collection("functions/tests/users").doc("1").onSnapshot(function(doc) {
    console.log(doc.name);
    document.getElementById("singleField").firstChild.nodeValue = doc.data().name + '--' + doc.data().year;
  });
}

FriendlyEats.prototype.signOut = function() {
...
FriendlyEats.prototype.onAuthStateChanged = function(user) {
  if(user) {
